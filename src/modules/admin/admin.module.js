...
const { BankAccountRepository } = require('../common/repositories');
...
class AdminModule {
  ...
  getListWithdrawalsUseCase() {
    if (!this._listWithdrawalsUseCase) {
      this._listWithdrawalsUseCase = new ListWithdrawalsUseCase({
        withdrawalRepository: new WithdrawalRepository(),
        userRepository: this.getUserRepository(),
        walletRepository: new WalletRepository()
      });
    }
    return this._listWithdrawalsUseCase;
  }

  getGetWithdrawalDetailsUseCase() {
    if (!this._getWithdrawalDetailsUseCase) {
      this._getWithdrawalDetailsUseCase = new GetWithdrawalDetailsUseCase({
        withdrawalRepository: new WithdrawalRepository(),
        userRepository: this.getUserRepository(),
        walletRepository: new WalletRepository(),
