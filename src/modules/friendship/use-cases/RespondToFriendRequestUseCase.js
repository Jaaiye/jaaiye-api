...
const { FriendRequestNotFoundError, FriendRequestExpiredError, InvalidFriendRequestActionError } = require('../errors');
...
class RespondToFriendRequestUseCase {
  ...
  async execute(userId, requestId, action) {
    if (!['accept', 'decline'].includes(action)) {
      throw new InvalidFriendRequestActionError('Action must be either "accept" or "decline"', 400);
    }

    const friendRequest = await this.friendRequestRepository.findById(requestId);
    if (!friendRequest) {
      throw new FriendRequestNotFoundError('Friend request not found', 404);
    }

    // Check if user is the recipient
    if (!friendRequest.isRecipient(userId)) {
      throw new BadRequestError('You can only respond to friend requests sent to you', 400);
    }

    if (!friendRequest.isPending()) {
      throw new BadRequestError('Friend request has already been processed', 400);
    }

    if (friendRequest.isExpired()) {
      throw new FriendRequestExpiredError('Friend request has expired', 400);
    }
